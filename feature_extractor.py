...
def basic_unit_with_downsampling(
    ...
    right_path = SeparableConv2D(
        in_channels, kernel_size=3, strides=stride, dilation_rate=rate, padding="same"
    )(right_path)
    right_path = batch_norm(right_path)
    right_path = Conv2D(
        out_channels // 2,
...
def basic_unit(inputs: tf.Tensor, rate: int = 1):
    splits = Lambda(lambda x: tf.split(x, num_or_size_splits=2, axis=3))(inputs)
    ...
    x = batch_norm(x)
    x = SeparableConv2D(
        in_channels, kernel_size=3, strides=1, dilation_rate=rate, padding="same"
    )(x)
    x = Conv2D(
        in_channels,
        kernel_size=1,
